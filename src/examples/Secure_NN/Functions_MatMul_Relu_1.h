#ifndef FUNCTIONS_MATMUL_RELU_1_H
#define FUNCTIONS_MATMUL_RELU_1_H
...
#include "GlobalVar_MatMul_Relu_1.h"
...
void read_shares(int choice, std::vector<uint8_t>&message, const Options& options) { 
  ...
  if (choice == 1) {
    ...
    file.close();
    std::cout << wpublic.size() << " " << wsecret.size() << " " << bpublic.size() << " " << bsecret.size() << std::endl; 
  }
  else if(choice==2)
  {
    std::string fullpath = options.current_path;
