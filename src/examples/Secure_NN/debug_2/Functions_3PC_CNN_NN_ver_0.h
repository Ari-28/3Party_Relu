#ifndef FUNCTIONS_CNN_NN_VAR_0_H
#define FUNCTIONS_CNN_NN_VAR_0_H
...
void read_shares(int choice, int layer_id, std::vector<uint8_t>&message, const Options& options, bool ConvToMatMulFlag) { 
  ...
  // Reading the weight and bias shares 
  if (choice == 1 && layer_type == 1) { 
    ...
    try {
      ...
      std::cout << "conv_kernels: " << conv_kernels << " " << conv_channels << " " << conv_rows << " " << conv_cols << "\n";
    ...
    file.close();
    std::cout << "Bias shares read." << std::endl;
