...
#include "Functions_3PC_CNN_NN_ver_1.h"
    ...
    case MatrixMulAddOpMessage:
      {
        ...
        message.erase(message.begin());
          ...
          if (party_id == helpernode_id) {
            ...
            __gnu_parallel::transform(public_shares_begin, public_shares_end, z_begin, public_shares_begin , std::plus{});   
            ...
            for (int i = 0; i < A_MatMul_Public_Shares.size(); i++) {
              A_MatMul_Private_Shares.push_back(randomnum[i]);
            }
            matrix_add_mul_flag_0 = true;
          }    
          break;
        }
   }
