// ./bin/3PC_Conv_Relu_2 --party 0,127.0.0.1,4009 --party 1,127.0.0.1,4010 --helper_node 127.0.0.1,4011 --current-path ${BASE_DIR}/build_debwithrelinfo_gcc
#include "Functions_3PC_CNN_NN_ver_2.h"
#include "GlobalVar_3PC_CNN_NN_ver_2.h"
...
class TestMessageHandler : public MOTION::Communication::MessageHandler {
  void received_message(std::size_t party_id, std::vector<std::uint8_t>&&message) {
   ...
   break;
    ...
    {
        message.erase(message.begin());
        std::cerr << "ConvolutionOpMessage DEBUG" << std::endl;
        int size_msg = message.size() / 8;
