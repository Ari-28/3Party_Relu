...
if [ ${layer_types[layer_id]} -eq 0 ] && [ $layer_id -eq 1 ];then 
  ...
  ./bin/3PC_MatMul_Relu_1 --WB_file file_config_model1 --input_file remote_image_shares --layer-id 1 --party 0,$cs0_host,$cs0_port_inference --party 1,$cs1_host,$cs1_port_inference --helper_node $helpernode_host,$helpernode_port_inference --current-path ${BASE_DIR}/build_debwithrelinfo_gcc > $debug_1/server1_MatMul_Relu_${layer_id}.txt &
  pid1=$!
  wait $pid1
  check_exit_statuses $?
  ...
  ./bin/3PC_Conv_Relu_1 --WB_file file_config_model1 --input_file remote_image_shares --layer-id 1 --party 0,$cs0_host,$cs0_port_inference --party 1,$cs1_host,$cs1_port_inference --helper_node $helpernode_host,$helpernode_port_inference --current-path ${BASE_DIR}/build_debwithrelinfo_gcc &
  pid1=$!
  wait $pid1
  check_exit_statuses $?

  echo "Layer ${layer_id}: Convolution and ReLU is done."

elif [ ${layer_types[layer_id]} -eq 1 ] && [ $layer_id -gt 1 ];then
   
   input_config="outputshare"

  ./bin/3PC_Conv_Relu_1 --WB_file file_config_model1 --input_file $input_config --layer-id $layer_id --party 0,$cs0_host,$cs0_port_inference --party 1,$cs1_host,$cs1_port_inference --helper_node $helpernode_host,$helpernode_port_inference --current-path ${BASE_DIR}/build_debwithrelinfo_gcc > $debug_1/server1_Conv_Relu_${layer_id}.txt &
  pid1=$!
  wait $pid1
  check_exit_statuses $?
  ...
  echo "Layer ${layer_id}: Convolution and ReLU is done."
   ...
   input_config="cnn_outputshare"
      ...
      $build_path/bin/3PC_MatMul_Relu_1 --WB_file file_config_model1 --input_file $input_config --party 0,$cs0_host,$cs0_port_inference --party 1,$cs1_host,$cs1_port_inference --helper_node $helpernode_host,$helpernode_port_inference --current-path $build_path --layer-id $layer_id --fractional-bits $fractional_bits > $debug_1/server1_MatMul_Relu_${layer_id}.txt &
      pid1=$!
      wait $pid1
      check_exit_statuses $?
      echo "Layer $layer_id: Matrix multiplication and ReLU is done."
