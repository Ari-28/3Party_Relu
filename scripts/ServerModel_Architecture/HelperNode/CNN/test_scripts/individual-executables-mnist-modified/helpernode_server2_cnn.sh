...
if [ ${layer_types[layer_id]} -eq 1 ];then 
   ...
   $build_path/bin/3PC_Conv_Relu_2 --party 0,$cs0_host,$cs0_port_inference --party 1,$cs1_host,$cs1_port_inference --helper_node $helpernode_host,$helpernode_port_inference --current-path ${BASE_DIR}/build_debwithrelinfo_gcc > $debug_2/helpernode_layer${layer_id}.txt &
   pid1=$!

   wait $pid1
   check_exit_statuses $?

   echo "Helper node layer $layer_id (Convolution + ReLU) is done"
...
# The final layer not involving ReLU
   ...
   $build_path/bin/3PC_MatMul_Relu_2 --party 0,$cs0_host,$cs0_port_inference --party 1,$cs1_host,$cs1_port_inference --helper_node $helpernode_host,$helpernode_port_inference --current-path $build_path > $debug_2/helpernode_layer${layer_id}.txt &
   pid2=$!
   wait $pid2
   check_exit_statuses $?
   echo "Helper node layer $layer_id is done"
